...
#include "PokemonHome_IvJudgeReader.h"
...
const IvJudgeReader& IV_READER(){
    const static Pokemon::IvJudgeReader reader("PokemonHome/IVCheckerOCR.json");
    return reader;
}
...
IvJudgeReaderScope::IvJudgeReaderScope(VideoOverlay& overlay, Language language)
    : m_language(language)
    //  Crop boxes are seeded from PokemonSV coords; HOME's hexagonal stat chart
    //  is nearly identical. Fine-tuned on hardware in Task 6.
